...
if [[ $# -lt 11 ]];then
    echo
    echo "usage: sh $0 -m maskFile -1 read1 -2 read2 -g indexedGenome -a annotationFile -o outDir -i image -t threads
...
then
    ...
    docker run --rm --cpus=64 --memory=80g \
      ...
      -v ${refDIR}:/refDir \
      -v ${imagDIR}:/imgDir \
    stomics/saw:04.1.0 /bin/bash register \
        -i ${image4register} \
        -c ${imageQC} \
        -v ${geneExp} \
        -o ${result_03register} && \
