...
# Rscript a1.2_qc_after_binning.R [rds with raw matrix] [nFeature cutoff] [nCount cutoff] [percent.mt cutoff]
...
if (length(args)<4) {
  stop("Usage: Rscript a1.3_qc_filter_after_binning.R [bined matrix] [nFeature cutoff] [nCount cutoff] [percent.mt cutoff]")
}
...
pMTCut<-as.numeric(args[4])

SeuObj<-subset(SeuObj0, subset = nFeature_RNA > nFeatureCut & nCount_RNA > nCountCut & percent.mt < pMTCut)

message(">>>")
message(paste0("Filtering conditions: nFeature > ", nFeatureCut, " and nCount > ", nCountCut, " and percent.mt < ", pMTCut))
...
saveRDS(SeuObj, file=paste0(args[1],"_nGene", args[2], "_nCount", args[3], "_mt", args[4], ".RDS"))

pdf(paste0(args[1], "_nGene", args[2], "_nCount", args[3], "_mt", args[4], "_QC.pdf"), width=8, height=4)
p1<-FeatureScatter(SeuObj, feature1 = "nCount_RNA", feature2 = "nFeature_RNA")+NoLegend()
...
p3<-FeatureScatter(SeuObj, feature1 = "nFeature_RNA", feature2 = "percent.mt")+NoLegend()
...
#  labs(fill="nGene",x=NULL,y=NULL,title = "spatial distribution of gene counts")+
#  theme(axis.title.x = element_text(size = 15),axis.title.y = element_text(size = 15) )+ 
#  theme(plot.title = element_text(hjust =0.5,vjust = 0.5,size = 15))
...
#    coord_fixed()+
#    scale_fill_gradient(low = "palegoldenrod", high = "magenta4")+
#    labs(fill="nUMI",x=NULL,y=NULL,title = "spatial distribution of read counts")+
#    theme(axis.title.x = element_text(size = 15),axis.title.y = element_text(size = 15) )+    
#    theme(plot.title = element_text(hjust =0.5,vjust = 0.5,size = 15))
